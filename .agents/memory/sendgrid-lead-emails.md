...
- As of 2026-07-28, `LEAD_NOTIFY_FROM_EMAIL` in dev is malformed (contains a space, no @) — SendGrid returns 400 "from email does not contain a valid address" for all sends (lead notifications and subscriber welcome emails) until it's set to a real, verified address.
