/**
 * Full all-pairs Phase 2 corpus similarity audit.
 *
 * Only the 400 Phase 2 MSA pages are compared. The threshold is inclusive:
 * 0.85 fails.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { PHASE2_METROS, PHASE2_SERVICE_SLUGS } from "./phase2-metros";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const PUBLIC = path.resolve(__dirname, "../public");
const REPORT = path.join(__dirname, "reports", "phase2-similarity.json");
const LIMIT = 0.85;
...
function pages(): CorpusPage[] {
  return PHASE2_METROS.flatMap((metro) => [
    `/metros/${metro.slug}/`,
    ...PHASE2_SERVICE_SLUGS.map((service) => `/metros/${metro.slug}/${service}/`),
  ]).map((url) => {
    const file = path.join(PUBLIC, url.replace(/^\/|\/$/g, ""), "index.html");
    if (!fs.existsSync(file)) throw new Error(`Phase 2 similarity audit missing page: ${url}`);
    const html = fs.readFileSync(file, "utf8");
    if (!html.includes('data-phase2="true"')) throw new Error(`Phase 2 page is not marked for audit: ${url}`);
...
function main() {
  const corpus = pages();
  const expected = PHASE2_METROS.length * 8;
  if (corpus.length !== expected) throw new Error(`Phase 2 similarity audit expected ${expected} pages, found ${corpus.length}`);
  ...
  if (violations.length) {
    throw new Error(`Phase 2 similarity audit failed: ${violations.length} pair(s) at or above 85%; see seo/reports/phase2-similarity.json`);
  }
}
