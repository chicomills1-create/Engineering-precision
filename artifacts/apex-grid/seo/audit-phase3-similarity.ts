/**
 * Full all-pairs Phase 3 corpus similarity audit.
 *
 * Only the 400 Phase 3 MSA pages are compared. The threshold is inclusive:
 * 0.85 fails. The report is deterministic so it can be reviewed in source
 * control without a wall-clock timestamp.
 */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { PHASE3_METROS, PHASE3_SERVICE_SLUGS } from "./phase3-metros";
...
const REPORT = path.join(__dirname, "reports", "phase3-similarity.json");
const LIMIT = 0.85;
...
function pages(): CorpusPage[] {
  return PHASE3_METROS.flatMap((metro) => [
    `/metros/${metro.slug}/`,
    ...PHASE3_SERVICE_SLUGS.map((service) => `/metros/${metro.slug}/${service}/`),
  ]).map((url) => {
    const file = path.join(PUBLIC, url.replace(/^\/|\/$/g, ""), "index.html");
    if (!fs.existsSync(file)) throw new Error(`Phase 3 similarity audit missing page: ${url}`);
    const html = fs.readFileSync(file, "utf8");
    if (!html.includes('data-phase3="true"')) throw new Error(`Phase 3 page is not marked for audit: ${url}`);
...
function main() {
  const corpus = pages();
  const expected = PHASE3_METROS.length * 8;
  if (corpus.length !== expected) throw new Error(`Phase 3 similarity audit expected ${expected} pages, found ${corpus.length}`);
  ...
  if (violations.length) {
    throw new Error(`Phase 3 similarity audit failed: ${violations.length} pair(s) at or above 85%; see seo/reports/phase3-similarity.json`);
  }
}
