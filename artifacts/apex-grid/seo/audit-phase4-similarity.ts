/**
 * Full all-pairs Phase 4 corpus similarity audit.
 *
 * Only the 400 Phase 4 MSA pages are compared. The threshold is inclusive:
...
import { PHASE4_METROS, PHASE4_SERVICE_SLUGS } from "./phase4-metros";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const PUBLIC = path.resolve(__dirname, "../public");
const REPORT = path.join(__dirname, "reports", "phase4-similarity.json");
const LIMIT = 0.85;
...
function pages(): CorpusPage[] {
  return PHASE4_METROS.flatMap((metro) => [
    `/metros/${metro.slug}/`,
    ...PHASE4_SERVICE_SLUGS.map((service) => `/metros/${metro.slug}/${service}/`),
  ]).map((url) => {
    const file = path.join(PUBLIC, url.replace(/^\/|\/$/g, ""), "index.html");
    if (!fs.existsSync(file)) throw new Error(`Phase 4 similarity audit missing page: ${url}`);
    const html = fs.readFileSync(file, "utf8");
    if (!html.includes('data-phase4="true"')) throw new Error(`Phase 4 page is not marked for audit: ${url}`);
...
function main() {
  const corpus = pages();
  const expected = PHASE4_METROS.length * 8;
  if (corpus.length !== expected) throw new Error(`Phase 4 similarity audit expected ${expected} pages, found ${corpus.length}`);
  ...
  if (violations.length) {
    throw new Error(`Phase 4 similarity audit failed: ${violations.length} pair(s) at or above 85%; see seo/reports/phase4-similarity.json`);
  }
}
