...
export const arizona: StateData = {
  ...
  narratives: {
    mep:
      "Mechanical design in Arizona is dominated by the cooling load: Valley projects see 115°F+ design days, so equipment selections carry high-ambient derates that out-of-state engineers routinely miss — a rooftop unit that meets load in Denver can fall 10–15% short in Phoenix in July. We size for ASHRAE 0.4% design conditions with ambient derating, specify MERV upgrades for monsoon dust seasons, and lean on the desert's dry air for evaporative and economizer strategies that cut energy cost on warehouses and data-adjacent shells. Because Apex Grid is headquartered in Queen Creek, our Arizona MEP work also reflects local utility realities — SRP and APS demand-charge structures reward load-shifting and right-sized electrical services.",
    structural:
